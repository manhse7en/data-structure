...
public class BinaryTree<E extends Comparable<E>> {
	...
	public Node<E> root() {
		/*
		 * If tree is empty, return null else return return root node
		 */
		if (isEmpty()) {
			return null;
		} else {
	...
	private int size(Node<E> root) {
		/*
		 * If root is null, return 0 else size of left subtree + size of right subtree +
		 * 1 for root
		 */
		if (root == null) {
			return 0;
	...
	private Node<E> findParent(E data, Node<E> root, Node<E> parent) {
		...
		/*
		 * Compare the root with the data. If they are not equal, first find parent in
		 * left subtree, if still parent is not found check right subtree
		 */
	...
	/**
	 * Method to get depth of the tree This is considered as Level as well
	 * 
	 * @param node
	 * @return {@link int}
	 */
	public int getDepth(Node<E> node) {
		...
		/*
		 * Get the depth of left and right. Whichever is greater, return that and add
		 * one for root
		 */
	...
	/**
	 * Method to print the tree InOrder InOrder : Left -> Root -> Right
	 * 
	 * @param node
	 */
	public void printInOrder(Node<E> node) {
		/*
		 * If root is null, return else print in order specified above
		 */
		if (node == null) {
			return;
		}
	...
	/**
	 * Method to print the tree PreOrder PreOrder : Root -> Left -> Right
	 * 
	 * @param node
	 */
	...
	/**
	 * Method to print the tree PostOrder PostOrder : Left -> Right -> Root
	 * 
	 * @param node
	 */
	 ...
	 * 
	 * Approach => Use 2 queues, Keep polling from queue 1 and keep adding to queue
	 * 2. Now, for next level, keep polling from queue 2 and keep adding from queue
	 * 1. Stop when both queues are empty. Basically elements at any level will be
	 * in one queue
	 * 
	 * @param root
	 */
	 ...
	 * Method to print the tree level by level
	 * 
	 * Approach => Use one queue and a delimiter i.e null. Add delimiter after every
	 * level. As soon as you encounter a null, print a new line and add null to the
	 * end of the queue
	 * 
	 * @param root
	 */
	public void printByLevelUsingOneQueueAndDelimiter(Node<E> root) {
		...
		/*
		 * Define a queue, add root to it. Since root itself is a level, add null
		 */
		...
		while (!queue.isEmpty()) {
			/*
			 * Poll the node and print its content. Keep adding elements from the next level
			 */
			root = queue.poll();
			if (root != null) {
				System.out.println(root.data + " ");
	 ...
	 * Method to print the tree level by level
	 * 
	 * Approach => Use one queue with count. Keep count of nodes at every level. As
	 * soon as this count is 0 print a new line.
	 * 
	 * @param root
	 */
	public void printByLevelUsingOneQueueAndCounter(Node<E> root) {
		...
		while (!queue.isEmpty()) {
			while (levelCount > 0) {
				/*
				 * Get the data, print the content and add children. Increase the element
				 * counter and decrease the level counter
				 */
				root = queue.poll();
				System.out.println(root.data + " ");
				if (root.left != null) {
			...
			/*
			 * Print new line, make level counter to current counter and set current counter
			 * to 0
			 */
			System.out.println();
